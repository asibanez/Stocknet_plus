...
def run_epoch_f(args, mode, model, criterion, optimizer,
    ...
    for step_idx, (X, Y) in tqdm(enumerate(data_loader),
        ...
        Y_gr_truth += Y.cpu().detach().numpy().tolist()
...
def main():
    ...
    if args.task == 'Train':
        ...
        if eval(args.train_toy_data):
            train_dataset = train_dataset[0:args.len_train_toy_data]
            dev_dataset = dev_dataset[0:args.len_train_toy_data]
        # Instantiate dataclasses
        train_dataset = model_module.stocknet_dataset(train_dataset)
        dev_dataset = model_module.stocknet_dataset(dev_dataset)
        ...
        print(f'{datetime.datetime.now()}: Done')
        # Instantiate dataclasses
        test_dataset = model_module.stocknet_dataset(test_dataset)
        # Instantiate dataloaders
    ...
    # Instantiate model
    model = model_module.stocknet_model(args)
    ...
    if args.task == 'Train':
        ...
        for epoch in tqdm(range(args.n_epochs), desc='Training dataset'):
            ...
            # Save checkpoint
            if eval(args.save_model_steps) and epoch >= args.save_step_cliff:
                utils.save_checkpoint_f(args, epoch, model, optimizer,
                                        train_loss)

        # Save model
        if eval(args.save_final_model):
            utils.save_checkpoint_f(args, epoch, model, optimizer, train_loss)

        # Save train results
        utils.save_train_results_f(args, train_loss_history,
